...
#### Full:
The memory is considered full when the write pointer plus one equals the read pointer (i.e., the write pointer is exactly one position behind the read pointer):
...
* de esta manera se simplifica la logica y se puede resetear los punteros automaticamente

* ademas no hace falta otro registro contador para hacer para saber cuando resetear, ahorrando espacio al no nesesitar mas registros (especialmente cuando la memoria es grande)

* codigo mas simple y elegante (menos posibilidad de fallo a alta frecuencia, tambien menor consumo energetico)
